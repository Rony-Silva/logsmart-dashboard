...
<style>
    ...
    /* Card de Detalhes do Item Selecionado */
    .detail-card {
...
if df_main.empty:
    ...
    # -------------------------------------------------------------------------
    # 4. Alertas e Inspeção Interativa por Clique do Item Zerado
    # -------------------------------------------------------------------------
    if qtd_zerados > 0:
        st.markdown(f"""
        <div class="alert-box">
        ...
        st.subheader("🚨 Inspeção Interativa de Itens Zerados")
        st.info("👆 **Como usar:** Clique em qualquer linha da tabela para selecionar e abrir a ficha completa do item zerado abaixo.")
        ...
        # Tabela com ativação de seleção por clique na linha
        event = st.dataframe(
        ...
        # Identificação da linha clicada pelo usuário
        selected_row_idx = 0
        if hasattr(event, 'selection') and event.selection and 'rows' in event.selection and len(event.selection['rows']) > 0:
            selected_row_idx = event.selection['rows'][0]
            
        # Garante índice válido
        if selected_row_idx >= len(itens_zerados):
            selected_row_idx = 0
            
        item_zerado_clicado = itens_zerados.iloc[selected_row_idx]
        
        # Exibição da Ficha do Item Clicado
        st.markdown("### 🔎 Ficha do Item Selecionado")
        
        col_f1, col_f2, col_f3 = st.columns(3)
        with col_f1:
            st.markdown(f"**🚢 Plataforma:** `{item_zerado_clicado['Plataforma']}`")
            st.markdown(f"**🏷️ Código do Item:** `{item_zerado_clicado['Codigo_Item'] if item_zerado_clicado['Codigo_Item'] else 'Não Informado'}`")
        with col_f2:
            st.markdown(f"**📦 Descrição:** `{item_zerado_clicado['Descricao'] if item_zerado_clicado['Descricao'] else 'Não Informada'}`")
            st.markdown(f"**📁 Categoria:** `{item_zerado_clicado['Categoria'] if item_zerado_clicado['Categoria'] else 'Não Informada'}`")
        with col_f3:
            st.markdown(f"**📍 Localização:** `{item_zerado_clicado['Localizacao'] if item_zerado_clicado['Localizacao'] else 'Não Informada'}`")
            st.markdown(f"**📄 Arquivo de Origem:** `{item_zerado_clicado['Arquivo_Origem']}`")
            
        st.error(f"⚠️ **STATUS DE ESTOQUE:** Quantidade = {item_zerado_clicado['Quantidade']:.2f} (ESTOQUE ZERADO)")
        
        st.markdown("---")

    # -------------------------------------------------------------------------
    # 5. Exportação e Impressão do Relatório
    # -------------------------------------------------------------------------
