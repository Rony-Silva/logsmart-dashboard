...
# -----------------------------------------------------------------------------
# 1. Configuração do Layout e Estilos CSS
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="LogSmart - Gestão de Estoque por Plataforma",
    page_icon="⚓",
    layout="wide",
...
<style>
    ...
    .metric-title { color: #9ca3af; font-size: 0.8rem; font-weight: 600; text-transform: uppercase; margin-bottom: 6px; }
    .metric-value { color: #f3f4f6; font-size: 1.7rem; font-weight: 700; }
    ...
    .stTabs [data-baseweb="tab"] {
        height: 42px; background-color: #1a1d24; border-radius: 8px;
        color: #9ca3af; font-weight: 600; border: 1px solid #2d313e;
    }
    .stTabs [aria-selected="true"] {
        background-color: #4f46e5 !important; color: #ffffff !important; border-color: #6366f1 !important;
    }
</style>
""", unsafe_allow_html=True)

# -----------------------------------------------------------------------------
# 2. Leitura e Filtragem Rigorosa dos Dados (Sem Linhas Fantasmas)
# -----------------------------------------------------------------------------
FOLDER_PATH = "dados"
# Mapeamento de colunas: I=8, J=9, L=11, N=13, O=14
        ...
        try:
            ...
            for col in ['Codigo_Item', 'Descricao', 'Localizacao', 'Categoria']:
                df_selected[col] = (
                    ...
                    .astype(str)
                    .str.strip()
                    .replace({'nan': '', 'None': '', '<NA>': '', 'NoneType': ''})
                )
            
            # Converte quantidade numérica
            ...
            # FILTRO CRÍTICO: Descarta linhas onde Código AND Descrição estão vazios ou são cabeçalhos
            palavras_descartar = ['codigo', 'código', 'code', 'item', 'descrição', 'descricao', 'quantidade', 'qtd', '']
            
            is_valid_item = (
                (df_selected['Codigo_Item'].str.lower().isin(palavras_descartar) == False) | 
                (df_selected['Descricao'].str.lower().isin(palavras_descartar) == False)
            ) & (
                (df_selected['Codigo_Item'] != '') | (df_selected['Descricao'] != '')
            )
            
            df_selected = df_selected[is_valid_item].copy()
...
# -----------------------------------------------------------------------------
# 3. Painel de Controle Principal
# -----------------------------------------------------------------------------
...
if df_main.empty:
    ...
    st.info(f"Insira os arquivos na pasta local `{os.path.abspath(FOLDER_PATH)}` e recarregue a página.")
else:
    ...
    itens_zerados = df_filtered[df_filtered['Quantidade'] == 0].reset_index(drop=True)
    ...
    # -------------------------------------------------------------------------
    # 4. Alerta Crítico e Seleção Direta do Item Zerado
    # -------------------------------------------------------------------------
    if qtd_zerados > 0:
        st.markdown(f"""
        <div class="alert-box">
        ...
        st.subheader("🚨 Inspeção e Detalhamento de Itens Zerados")
        
        # Cria um rótulo legível para o seletor
        itens_zerados['Rotulo_Exibicao'] = (
            "🚢 " + itens_zerados['Plataforma'] + 
            " | Cod: " + itens_zerados['Codigo_Item'].replace('', 'S/N') + 
            " - " + itens_zerados['Descricao'].replace('', 'Sem Descrição')
        )
        
        # Seletor interativo
        opcao_item_selecionado = st.selectbox("🔎 Escolha um item zerado para ver a ficha completa:", options=itens_zerados['Rotulo_Exibicao'].tolist())
        
        item_zerado_focado = itens_zerados[itens_zerados['Rotulo_Exibicao'] == opcao_item_selecionado].iloc[0]
        
        # Exibição do Card Detalhado do Item Zerado
        st.markdown("""<div class="detail-card">""", unsafe_allow_html=True)
        st.markdown(f"#### 🛑 Ficha Detalhada do Item Zerado")
        
        f1, f2, f3 = st.columns(3)
        with f1:
            st.markdown(f"**Plataforma:** `{item_zerado_focado['Plataforma']}`")
            st.markdown(f"**Código do Item:** `{item_zerado_focado['Codigo_Item'] if item_zerado_focado['Codigo_Item'] else 'N/A'}`")
        with f2:
            st.markdown(f"**Descrição / Nome:** `{item_zerado_focado['Descricao'] if item_zerado_focado['Descricao'] else 'N/A'}`")
            st.markdown(f"**Categoria:** `{item_zerado_focado['Categoria'] if item_zerado_focado['Categoria'] else 'N/A'}`")
        with f3:
            st.markdown(f"**Localização:** `{item_zerado_focado['Localizacao'] if item_zerado_focado['Localizacao'] else 'N/A'}`")
            st.markdown(f"**Arquivo Fonte:** `{item_zerado_focado['Arquivo_Origem']}`")
            
        st.error(f"⚠️ **ESTOQUE ATUAL:** {item_zerado_focado['Quantidade']:.2f} (CRÍTICO / ZERADO)")
        st.markdown("""</div>""", unsafe_allow_html=True)

        # Tabela Consolidada de Todos os Itens Zerados
        with st.expander("📋 Expandir tabela completa de todos os itens zerados"):
            tabela_alerta_view = itens_zerados[['Plataforma', 'Codigo_Item', 'Descricao', 'Categoria', 'Localizacao', 'Quantidade']].copy()
            tabela_alerta_view.columns = ['Plataforma', 'Código do Item', 'Descrição do Item', 'Categoria', 'Localização', 'Qtd em Estoque']
            
            st.dataframe(
                tabela_alerta_view.style.map(lambda v: 'background-color: rgba(239, 68, 68, 0.25); color: #ef4444; font-weight: bold;' if v == 0 else '', subset=['Qtd em Estoque']),
    ...
    # -------------------------------------------------------------------------
    # 5. Relatório de Exportação e Impressão
    # -------------------------------------------------------------------------
